...
!function($s) {
  ...
  var drawCAP = function(g, x, y, width, height) {
    g.moveTo(x, y + height/2);
    g.lineTo(x + 2*width/5, y + height/2);
    g.moveTo(x + 2*width/5, y + height*2/3 );
    g.lineTo(x + 2*width/5, y + height/3);
    g.moveTo(x + 3*width/5, y + height*2/3);
    g.lineTo(x + 3*width/5, y + height/3);
    g.moveTo(x + 3*width/5, y + height/2);
    g.lineTo(x + height,y + height/2);
    g.closePath(false);
  };

  var drawGND = function(g, x, y, width, height) {
    g.moveTo(x + width, y + height/2);
    g.lineTo(x + 4*width/5, y + height/2);

    g.moveTo(x + 2*width/5, y + height*3.2/6 );
    g.lineTo(x + 2*width/5, y + height*2.7/6); 

    g.moveTo(x + 3*width/5, y + height*4/6);
    g.lineTo(x + 3*width/5, y + height*2/6);

    g.moveTo(x + 4*width/5, y + height*5/6);
    g.lineTo(x + 4*width/5, y + height/6);
  ...
  var createGND = function(op, out, draw) {
    return function(device) {
      device.addOutput();
      var inputs = device.getInputs();
      var outputs = device.getOutputs();
  ...
  */
  ...
  $s.registerDevice('CAP', createLogicGateFactory(null, NOT, drawCAP) );
  $s.registerDevice('GND', createGND(null, NOT, drawGND) );
