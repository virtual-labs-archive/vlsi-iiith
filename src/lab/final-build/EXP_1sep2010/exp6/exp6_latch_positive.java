...
public class exp6_latch_positive extends JApplet 
{
	public  void init()
	{
	...
	public  class MyPanel extends JPanel  implements ActionListener , MouseMotionListener 
	{
		...
		// 2InputMUX1 = 0-3 ; Inverter1 = 4-5 ;Inverter2 = 6-7; OUTPUT = 8 ; INPUT = D:9 , clk: 10 ;
					...
					for ( int k = 0 ; k < 4 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] + 5; i ++ )
						{
							for ( int j = end_pointsY[k] - 4 ; j < end_pointsY[k] +5; j ++ )
							{
								...
								end_points_mat[i][j] = k ; // 0-3 for  2InputMUX ; 
							}
						}
					}
				}
				else if ( img == 4 || img == 10 ) //  INPUT , OUTPUT
				{
					...
					for ( int i = end_pointsX[0] - 4 ; i < end_pointsX[0] +5 ; i ++ )
					{
						for ( int j = end_pointsY[0] - 4 ; j < end_pointsY[0] +5; j ++ )
						{
							if ( img == 4 ) 				// INPUT
							{
								end_points_mat[i][j] = 8  + comp_no  ; // 9-10 ( D , clk  )
				...
				else if ( img == 11 ) // Inverter 
				{
					for ( int k = 0 ; k < 2 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] +5 ; i ++ )
						{
							for ( int j = end_pointsY[k] - 4 ; j < end_pointsY[k] +5; j ++ )
							{
								end_points_mat[i][j] = 4 + k + 2*(comp_no - 1); // 4-5 Inverter1 and 6-7 Inverter2  
													   // as comp_no startsfrom 1	
							}
						}
					}
		...
		public class myDialog extends JDialog implements ActionListener 
		{
			...
			public myDialog (JFrame fr , String comp, int node_no)
			{
				...
				else if ( comp_node[node_no].img_no == 11 ) // Inverter 
				{
					...
					JLabel comp_name = new JLabel("<html><font size=5><b>"+comp+" "+comp_node[node_no].comp_no+"</b></font></html>" );//,icon[icon_no],JLabel.CENTER);
					

					layout.putConstraint(SpringLayout.WEST , comp_name , 100,   SpringLayout.WEST , cp );
					layout.putConstraint(SpringLayout.NORTH , comp_name , 40,  SpringLayout.NORTH , cp);
			...
			public void mouseDragged(MouseEvent me)
			{
				...
				else 
				{
					...
					/*	for ( i = comp_node[total_comp-1].node_x ;  ;)
						...
						for ( i = work_x -80; i < work_x + comp_node[node_drag].width +60; i++ ) 
						{
							for ( j = work_y -80 ; j < work_y + comp_node[node_drag].height+60 ; j++ )
							{
...
//				g.drawRect( x , y , 2*width , 2*width);
		...
		{
			String fileToRead = "outfile_latch_p";
...
//Setting Top Panel ========================================================================== 
			...
			JLabel heading = new JLabel (  "<html><FONT COLOR=WHITE SIZE=18 ><B> D-LATCH : Positive LEVEL</B></FONT></html>", JLabel.CENTER);
			...
			String[] my_list = {"D Flip-Flop : Positive Edge " , "D-Latch : Positive Level"};
...
//			bottom.add(h );
		...
		public boolean circuit_check()
		{
			...
			int temp ;
			for (  i = 0 ; i < 11 ; i ++ ) // for making aal connected point for each end_point in its array ..
			{	
...
*/			// Used Replaced array only for indexies ( 0-23 ) i.e NPOS and PMOS
			...
			{
				l = 0 ;
				while (  check_points_value[0][l] != -1 )  // clk 
				{
					if ( check_points_value[0][l] != 0 && check_points_value[0][l] != 10 )
					{
				...
				l = 0 ;
				while (  check_points_value[9][l] != -1 )  // IN ( D)
				{
				...
				{
					if ( check_points_value[8][l] != 8 && check_points_value[8][l] != 5 && check_points_value[8][l] != 6)
					{
				...
				l = 0 ;
				while (  check_points_value[1][l] != -1 )  // MUX out
				{
					if ( check_points_value[1][l] != 1 &&  check_points_value[1][l] != 4)
					{
							System.out.println("Mux Out");			
						return false ;
					}
					l++;
				}
				...
				l = 0 ;
				while (  check_points_value[2][l] != -1 )  // Input2 of MUX
				{
					if ( check_points_value[2][l] != 2 &&  check_points_value[2][l] != 7 )
					{
							System.out.println("Input2 of MUX");			
						return false ;
					}
					l++;
				}
		...
		public void actionPerformed(ActionEvent e )
		{
			...
			if ( e.getSource() == exp_list )
			{
				...
				else 
				{
					...
					try
					{
						...
						URL url = new URL(base ,"exp6_flipflop_positive.html");
			...
			else if(e.getSource() == simulate_button )
			{
				...
				if (circuit_check())
				{
					...
					String      encoded = "comp=" + URLEncoder.encode(MinVolt1+"_"+MaxVolt1+"_"+RiseTime1+"_"+FallTime1+"_"+StepSize1+"_"+TotalTime1+"_"+MinVolt2+"_"+MaxVolt2+"_"+RiseTime2+"_"+FallTime2+"_"+StepSize2+"_"+TotalTime2); 
					URL CGIurl = null;
					try
					{
						if ( exp_type == 0 )
						{
							php_file = new URL(getDocumentBase(),"exp6_flipflop_positive_simulate.php");
							System.out.println( php_file.toString());
						}
						else if ( exp_type == 1 )
						{
							php_file = new URL(getDocumentBase(),"exp6_latch_positive_simulate.php");
							System.out.println( php_file.toString());
						}
					}
					catch(Exception mye )
...
//				if ( simulate_flag == true )
		...
		//		}
			...
			{
				if ( comp_count[4] < 2 ) // 2 inputs are allowed 
				{
