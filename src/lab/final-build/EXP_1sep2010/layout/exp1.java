	...
	public  class MyPanel extends JPanel  implements ActionListener , MouseMotionListener 
	{
		...
		public  class WorkPanel extends JPanel  implements  MouseMotionListener , MouseListener   
		{
			...
			public void mouseDragged(MouseEvent me)
			{
				...
				if ( comp_pick_mode == 2 && img_button_pressed != -1 ) // Streching is allowed 
				{
					...
					if ( node_strech == true )
					{
						...
						// Streacting can be done in +Ve X and Y direction 
						if ( each_comp_node_array[img_button_pressed][comp_count[img_button_pressed]-1].node_x > (work_x /width )*width || each_comp_node_array[img_button_pressed][comp_count[img_button_pressed]-1].node_y > (work_y /width )*width ){
							return ;
						}	



						// If the other component of same type is already there
						for(i = each_comp_node_array[img_button_pressed][comp_count[img_button_pressed]-1].node_x ; i < (work_x /width )*width ; i++){
							for(j = each_comp_node_array[img_button_pressed][comp_count[img_button_pressed]-1].node_y ; j < (work_y /width )*width ; j ++ ){

							//	System.out.print(work_mat_array[selected_mat][i][j]);
								if ( work_mat_array[img_button_pressed][i][j] != -1 &&  work_mat_array[img_button_pressed][i][j] != comp_count[img_button_pressed]-1 ){
				...
				else if (  comp_pick_mode == 0 && img_button_pressed == -1 && selected_mat != 0 && work_x >= 0 && work_y >= 0 ) // Move  is allowed 
				{
					...
					// If the other component of same type is already there
					for(i = (work_x /width )*width; i < (work_x /width )*width + each_comp_node_array[selected_mat][selected_index].width ;i++){
						for(j=(work_y /width )*width;j<(work_y/width )*width+ each_comp_node_array[selected_mat][selected_index].height ; j ++ ){

							//	System.out.print(work_mat_array[selected_mat][i][j]);
								if ( work_mat_array[selected_mat][i][j] != -1 && work_mat_array[selected_mat][i][j] != selected_index){
			...
			public void mouseClicked(MouseEvent me) 
			{
				...
				{	
					if ( work_mat_array[img_button_pressed][(work_x / width)*width ][ (work_y / width)*width ] != -1 ){
						return ;
					}
...
/*						
		...
		public void actionPerformed(ActionEvent e )
		{
			...
			else if(e.getSource() == check_button )
			{
				...
				if (img_button_pressed != -1 )  // If Check Button is pressed before fixing the Size Of component.
				{	
					JOptionPane.showMessageDialog(null, "Please First Fix the Selected Component Size.");
				}
				else	if ( check_spacing_same_metal() && check_overlapping_diff_metal() )
				{
...
// 				}
				...
				{
					JOptionPane.showMessageDialog(null, "This Component is not required in this Experiment. ");
