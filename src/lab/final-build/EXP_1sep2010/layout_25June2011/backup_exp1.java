...
//					System.out.println("node index is "+node_index);
		...
		{
			int width1[]={10,10,10,10,10,10,10,10,10,10};
			int width2[]={10,10,10,10,10,10,10,10,10,10};
			int pos_x[]={0,0};
			int pos_y[]={0,0};
			int c_x = 0;
			int c_y = 0;
...
//					wire[total_wire-1 ].update2((work_x/20)*20 , (work_y/20)*20);
			...
			public void mouseDragged(MouseEvent me)
			{
				...
				System.out.println("move is "+movestate);
				...
				if(movestate != 0 && stretching == 0)
				{
					...
					System.out.println("entered the return thing");
					...
					if (node_drag != -1 )
					{
						System.out.println("entered node_drag");
				...
				//	System.out.println("mouse x "+work_x+"mouse y "+work_y+"block x "+temp_x+"block y "+temp_y+" w1 "+width1[node_drag]+"w2 "+width2[node_drag]);
					if((work_x > temp_x ) && (work_x < temp_x + width1[node_drag] )&& (work_y > temp_y))
					{
						...
						{
							if(width2[node_drag] > (((-temp_y + work_y)/10))*10)
					...
					else if((temp_x + width1[node_drag] -2 < work_x ) && (temp_y  < work_y && work_y < temp_y + width2[node_drag] ))
					{
						...
						{
							if(width1[node_drag] > (((-temp_x + work_x)/10))*10)
...
//			}
			else  // For adding comp 
			{
					...
					System.out.print("img_button_pressed ");
			...
			public void paint(Graphics g) 
			{
				...
				Line2D line1 = new Line2D.Float(9,1,738,1);
				Line2D line2 = new Line2D.Float(8,8,8,566);
//			        g2d.draw(lin);
		...
		//For Images --------------------------------
			...
			void draw_metal1(Graphics2D g , int x , int y , int width1 ,int width2, double angle)
			{
				...
				g.rotate(angle , x , y);
				g.setColor(new Color(0,0,180,38));
...
//			Y = work_y;
		...
		//	movey2_button.addActionListener(this);
...
//Setting Top Panel ========================================================================== 
			...
			JLabel heading = new JLabel (  "<html><FONT COLOR=WHITE SIZE=18 ><B>LAYOUT OF INVERTER </B></FONT></html>", JLabel.CENTER);
