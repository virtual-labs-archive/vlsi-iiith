...
public class exp7_size extends JApplet 
{
	public  void init()
	{
	...
	public  class MyPanel extends JPanel  implements ActionListener , MouseMotionListener 
	{
		...
		String[] comp_str = {		// This will store what should at the Dialog Box for each component
			...
			"OUTPUT",  "Invertor " ,"Inductor ",  // 10, 11 , 12
			" " ," "};
		...
		// Input = 0 ; Output = 1 ; Ground = 2 ; Capacitor = 3-4 ; Inverter1 = 5-6 ;Inverter2 = 7-8; Inverter3 = 9-10 ;Inverter4 = 11-12 ;Inverter5 = 13-14 ;
...
// Input = 0 ; Output = 1 ; Ground = 2 ; Capacitor = 3-4 ; Inverter1 = 5-6 ;Inverter2 = 7-8; Inverter3 = 9-10 ;Inverter4 = 11-12 ;Inverter5 = 13-14 ;
				...
				{	
					for ( int k = 0 ; k < 2 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] +5 ; i ++ )
						{
							...
							{
								end_points_mat[i][j] = 3 + k  ; // 3-4 Capacitor 
							}
						}
					}	
				}
				else if ( img == 4 || img == 10 || img == 2) //  INPUT , OUTPUT , Gnd
				{
					...
					for ( int i = end_pointsX[0] - 4 ; i < end_pointsX[0] +5 ; i ++ )
					{
						for ( int j = end_pointsY[0] - 4 ; j < end_pointsY[0] +5; j ++ )
						{
							...
							{
								end_points_mat[i][j] =  2;		// Gnd
							}
						}
					}
				}
				else if ( img == 11 ) // Inverter 
				{
					for ( int k = 0 ; k < 2 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] +5 ; i ++ )
						{
							for ( int j = end_pointsY[k] - 4 ; j < end_pointsY[k] +5; j ++ )
							{
								end_points_mat[i][j] = 5 + k + 2*(comp_no - 1); // 5-6 Inverter1 ; 7-8 Inverter2 and so on
			...
			public void make_end_points(int img )
			{
				...
				if ( img == 5 ) // 2Input MUX
				{
					...
					end_pointsY[2] = node_y + (2* height)/3 ;

					end_pointsX[3] = node_x  ;
					end_pointsY[3] = node_y +  height / 3;
				...
				else if ( img == 8 || img == 11) // Capacitor and Inverter 
				{
					count_end_points = 2 ;
					int a , b , c , d  ;
...
//				end_index++;
		...
		public class myDialog extends JDialog implements ActionListener 
		{
			...
			JSpinner p_length ;
			JSpinner p_width;
			SpinnerModel p_length_model ;
			SpinnerModel p_width_model ;
			JSpinner n_length ;
			JSpinner n_width;
			SpinnerModel n_length_model ;
			SpinnerModel n_width_model ;
			...
			public myDialog (JFrame fr , String comp, int node_no)
			{
				...
				else if ( comp_node[node_no].img_no == 11 ) // Inverter 
				{
					p_length_model =	new SpinnerNumberModel(50, //initial value
							0, //min
							1000000, //max
							1);  //step
					p_width_model =	new SpinnerNumberModel(100, //initial value
							0, //min
							1000000, //max
							1);  //step
					n_length_model =	new SpinnerNumberModel(50, //initial value
							0, //min
							1000000, //max
							1);  //step
					n_width_model =	new SpinnerNumberModel(100, //initial value
							0, //min
							1000000, //max
							1);  //step
					...
					p_length = new JSpinner(p_length_model);
					p_width = new JSpinner(p_width_model);
		
					n_length = new JSpinner(n_length_model);
					n_width = new JSpinner(n_width_model);

					JLabel p_w = new JLabel("PMOS Width:");
					JLabel n_w = new JLabel("NMOS Width:");
...
////////////////////////////////////////////////////////////////////////////////////
					...
					layout.putConstraint(SpringLayout.WEST , n_w , 10,   SpringLayout.WEST , cp );
					layout.putConstraint(SpringLayout.NORTH , n_w , 50,  SpringLayout.NORTH , cp);
					layout.putConstraint(SpringLayout.WEST , n_width , 5,   SpringLayout.EAST , n_w );
					layout.putConstraint(SpringLayout.NORTH , n_width , 50,  SpringLayout.NORTH , cp);

					layout.putConstraint(SpringLayout.WEST , p_w , 20,   SpringLayout.EAST , n_width );
					layout.putConstraint(SpringLayout.NORTH , p_w , 50,  SpringLayout.NORTH , cp);
					layout.putConstraint(SpringLayout.WEST , p_width , 5,   SpringLayout.EAST , p_w );
					layout.putConstraint(SpringLayout.NORTH , p_width , 50,  SpringLayout.NORTH , cp);
				...
				//	layout.putConstraint(SpringLayout.NORTH , width_unit , 50,  SpringLayout.NORTH , cp);
					...
					JLabel p_l = new JLabel("PMOS Length:");
					JLabel n_l = new JLabel("NMOS Length:");
				//	length_unit = new JComboBox(unit);
					//length_unit.addActionListener(this);
					//length_unit.setSelectedIndex(2);
					

					layout.putConstraint(SpringLayout.WEST , n_l , 5,   SpringLayout.WEST , cp );
					layout.putConstraint(SpringLayout.NORTH , n_l , 80,  SpringLayout.NORTH , cp);
					layout.putConstraint(SpringLayout.WEST , n_length , 5,   SpringLayout.EAST , n_l );
					layout.putConstraint(SpringLayout.NORTH , n_length , 80,  SpringLayout.NORTH , cp);

					layout.putConstraint(SpringLayout.WEST , p_l , 10,   SpringLayout.EAST , n_length );
					layout.putConstraint(SpringLayout.NORTH , p_l , 80,  SpringLayout.NORTH , cp);
					layout.putConstraint(SpringLayout.WEST , p_length , 5,   SpringLayout.EAST , p_l );
					layout.putConstraint(SpringLayout.NORTH , p_length , 80,  SpringLayout.NORTH , cp);
			...
			//		del.addActionListener(this);
				...
				else if (comp_node[node_no].img_no == 4 ) // INPUT 
				{
					...
					{
						comp_name = new JLabel("<html><font size=5><b>"+comp+" In </b></font></html>" );//,icon[icon_no],JLabel.CENTER);
					}
					...
					layout.putConstraint(SpringLayout.WEST , comp_name , 80,   SpringLayout.WEST , cp );
					layout.putConstraint(SpringLayout.NORTH , comp_name , 20,  SpringLayout.NORTH , cp);
...
//					length.setValue(45);
		//			length_model.setValue(45);
			...
			String get_p_length() 
			{
				if (  Integer.parseInt(p_length.getValue().toString()) <= 45 ) // nano 
				{
					JOptionPane.showMessageDialog(null, "Length value is less the 45n , this is not allowed by this technology .");
//					length.setValue(45);
		//			length_model.setValue(45);
				...
				{
					return p_length.getValue().toString()+"n";
				}
				//return length.getValue().toString()+(String)length_unit.getSelectedItem();
			}
			String get_p_width()
			{
				...
				if ( Integer.parseInt(p_width.getValue().toString()) <= 90 ) // nano 
				{
					JOptionPane.showMessageDialog(null, "Width value is less the 90n , this is not allowed by this technology .");
					return "90n" ;
				}
				...
				{
					return p_width.getValue().toString()+"n";
				}
				//return width.getValue().toString()+(String)width_unit.getSelectedItem();
			}
			String get_n_length() 
			{
				if (  Integer.parseInt(n_length.getValue().toString()) <= 45 ) // nano 
				{
					JOptionPane.showMessageDialog(null, "Length value is less the 45n , this is not allowed by this technology .");
//					length.setValue(45);
		//			length_model.setValue(45);
				...
				{
					return n_length.getValue().toString()+"n";
				}
				//return length.getValue().toString()+(String)length_unit.getSelectedItem();
			}
			String get_n_width()
			{
				...
				if ( Integer.parseInt(n_width.getValue().toString()) <= 90 ) // nano 
				{
					JOptionPane.showMessageDialog(null, "Width value is less the 90n , this is not allowed by this technology .");
					return "90n" ;
				}
				...
				{
					return n_width.getValue().toString()+"n";
				}
				//return width.getValue().toString()+(String)width_unit.getSelectedItem();
			}
			...
			public void actionPerformed(ActionEvent e )
			{
				if(e.getSource() == ok )
				{
					...
					else if ( comp_node[node_index].img_no  == 11 ) // Inverter
					{
						if ( comp_node[node_index].comp_no == 1 ) // Inverter 1
						{
							 Pmos1_l = get_p_length() ;
							 Pmos1_w = get_p_width();
							 Nmos1_l = get_n_length();
							 Nmos1_w = get_n_width();	
						}
						else if ( comp_node[node_index].comp_no == 2 ) // Inverter 2
						{
							 Pmos2_l = get_p_length() ;
							 Pmos2_w = get_p_width();
							 Nmos2_l = get_n_length();
							 Nmos2_w = get_n_width();	
						}	
						else if ( comp_node[node_index].comp_no == 3 ) // Inverter 3
						{
							 Pmos3_l = get_p_length() ;
							 Pmos3_w = get_p_width();
							 Nmos3_l = get_n_length();
							 Nmos3_w = get_n_width();	
						}
						else if ( comp_node[node_index].comp_no == 4 ) // Inverter 4
						{
							 Pmos4_l = get_p_length() ;
							 Pmos4_w = get_p_width();
							 Nmos4_l = get_n_length();
							 Nmos4_w = get_n_width();	
						}
						else if ( comp_node[node_index].comp_no == 5 ) // Inverter 5
						{
							 Pmos5_l = get_p_length() ;
							 Pmos5_w = get_p_width();
							 Nmos5_l = get_n_length();
							 Nmos5_w = get_n_width();	
						}
					}
			...
			public void mouseClicked(MouseEvent me) 
			{
				...
				if ( img_button_pressed == -1 ) // for selecting anything on work panel
				{
					...
					else if ( work_mat[work_x][work_y]!= -1 ) // there is a comp on (work_x , work_y) here temp rep inder of comp_nodearray
					{
						...
						if (  temp1 == 1 || temp1 == 7 || temp1 == 8 || temp1 == 2 ||  temp1 == 9  || temp1 == 11 ||( /*for Input  */temp1 == 4 && (comp_node[temp].comp_no == 1 || comp_node[temp].comp_no == 2 ||comp_node[temp].comp_no == 3 )))
						{
...
*/
				...
				else  // For adding comp 
				{
					...
					else if (  img_button_pressed == 11 ) // Inverter
					{
						comp_node[total_comp] = new node((work_x / grid_size)*grid_size , (work_y / grid_size )*grid_size , img_button_pressed , grid_size * 3, 2*grid_size , comp_count[11] + 1);
					}
		...
		//For Images --------------------------------
				...
				for ( i = 0; i < total_comp ; i++ )
				{
					if ( comp_node[i].del != true )
					{
						...
						else if ( comp_node[i].img_no == 11) // Invertor 
						{
							draw_inverter(g2d , comp_node[i].node_x  , comp_node[i].node_y , grid_size , comp_node[i].angle);
							g.setColor(Color.yellow);
...
//				g.drawRect( x , y , 2*width , 2*width);
			...
			void draw_input(Graphics2D g , int x , int y , int width , double angle , int comp_no)
			{
				...
				if ( comp_no == 1 )
				{
					g.drawString("In" , x - 20 , y+ width - 2 );
				}
				else if ( comp_no == 2 )
				{
			...
			//	g.drawString("OUT", x  + 2* width + 2 , y+ width - 3);
				...
				//	g.drawLine(x + (5*width)/2 , y + 2, x +3*width ,y + 2 );
					g.drawString("Out", x +(5*width)/2 , y+ width );
		...
		{
			String fileToRead = "outfile_size";
			...
			public void make_graph()
			{
				...
				//	System.out.println("Hi I am in the gpaint func of the exp1_graph class :)");
					...
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					    ...
					    g2d.drawLine(40+2*i , 135+90-(int)Math.round(V_In[i]*70) , 40 + 2*(i+1) , 135+90-(int)Math.round(V_In[i+1]*70) );
					}
					g2d.drawString("In ",  4 , 120);
...
*/
					...
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					   g2d.drawLine(40+2*i , 320-(int)Math.round(V_out[i]*70) , 40 + 2*(i+1) , 320-(int)Math.round(V_out[i+1]*70) );
					}
				//	g2d.drawLine( 6 , 385-40 , 20 , 385-40);
					g2d.drawString("Out",  8 , 350 );
...
//Setting Top Panel ========================================================================== 
			...
			JLabel heading = new JLabel (  "<html><FONT COLOR=WHITE SIZE=18 ><B> GATE SIZING </B></FONT></html>", JLabel.CENTER);
...
//			bottom.add(h );
		...
		{
			int check_points_value[][] = new int[15][15] ; // each index will store corresponding  component for points of (comp point no -> index )
			...
			int temp ;
			for (  i = 0 ; i < 15 ; i ++ ) // for making aal connected point for each end_point in its array ..
			{	
...
// Input = 0 ; Output = 1 ; Ground = 2 ; Capacitor = 3-4 ; Inverter1 = 5-6 ;Inverter2 = 7-8; Inverter3 = 9-10 ;Inverter4 = 11-12 ;Inverter5 = 13-14 ;
			...
			if ( exp_type == 0 ) // Sizing
			{
				l = 0 ;
				while (  check_points_value[0][l] != -1 )  // In
				{
					if ( check_points_value[0][l] != 0 && check_points_value[0][l] != 5 )
					{
						System.out.println("In");			
						return false ;
					}
				...
				l = 0 ;
				while (  check_points_value[6][l] != -1 )  // Inverter 1-2
				{
					if ( check_points_value[6][l] != 6 &&  check_points_value[6][l] != 7 )
					{
						System.out.println("Inverter 1-2");			
						return false ;
					}
					l++;
				}
				l = 0 ;
				while (  check_points_value[8][l] != -1 )  // Inverter 2-3
				{
					if ( check_points_value[8][l] != 8 &&  check_points_value[8][l] != 9 )
					{
						System.out.println("Inverter 2-3");			
						return false ;
					}
					l++;
				}
				l = 0 ;
				while (  check_points_value[10][l] != -1 )  // Inverter 3-4
				{
					if ( check_points_value[10][l] != 10 &&  check_points_value[10][l] != 11 )
					{
						System.out.println("Inverter 3-4");			
						return false ;
					}
					l++;
				}
				l = 0 ;
				while (  check_points_value[12][l] != -1 )  // Inverter 4-5
				{
					if ( check_points_value[12][l] != 12 &&  check_points_value[12][l] != 13 )
					{
						System.out.println("Inverter 4-5");			
						return false ;
					}
					l++;
				}
				l = 0 ;
				while (  check_points_value[1][l] != -1 )  // Out
				{
					if ( check_points_value[1][l] != 1 &&  check_points_value[1][l] != 14  &&  check_points_value[1][l] != 3 &&  check_points_value[1][l] != 4)
					{
				...
				l = 0 ;
				while (  check_points_value[2][l] != -1 )  // Gnd
				{
					if ( check_points_value[2][l] != 2 && check_points_value[2][l] != 3 && check_points_value[2][l] != 4)
					{
			...
			System.out.println("End Of Check Code ");			
		        return false ;
		}
		...
		public void actionPerformed(ActionEvent e )
		{
			...
			if ( e.getSource() == exp_list )
			{
				...
				else 
				{
					...
					try
					{
						...
						URL url = new URL(base ,"exp7_size.html");
			...
			else if(e.getSource() == simulate_button )
			{
				...
				if (circuit_check())
				{
					...
					String      encoded = "comp=" + URLEncoder.encode(Capacitance+"_"+MinVolt1+"_"+MaxVolt1+"_"+RiseTime1+"_"+FallTime1+"_"+StepSize1+"_"+TotalTime1+"_"+Pmos1_l+"_"+Pmos1_w+"_"+Nmos1_l+"_"+Nmos1_w+"_"+Pmos2_l+"_"+Pmos2_w+"_"+Nmos2_l+"_"+Nmos2_w+"_"+Pmos3_l+"_"+Pmos3_w+"_"+Nmos3_l+"_"+Nmos3_w+"_"+Pmos4_l+"_"+Pmos4_w+"_"+Nmos4_l+"_"+Nmos4_w+"_"+Pmos5_l+"_"+Pmos5_w+"_"+Nmos5_l+"_"+Nmos5_w); 
					URL CGIurl = null;
					try
					{
						if ( exp_type == 0 )
						{
							php_file = new URL(getDocumentBase(),"exp7_size_simulate.php");
...
//				if ( simulate_flag == true )
		...
		//		}
			...
			else if(e.getSource() == graph_button )
			{
				...
				if ( simulate_flag == true )
				{
					try
					{
						...
						if ( exp_type == 0 )
						{
							url = new URL(base ,"exp6_flipflop_positive_fullgraph.php");
						}
						else if ( exp_type == 1 )
						{
							url = new URL(base ,"exp6_latch_positive_fullgraph.php");
						}
			...
			else if (e.getSource() == img_button1[2] ) // ground 
			{
				if ( comp_count[2] < 1 ) // 1 Ground are allowed 
				{
			...
			{
				if ( comp_count[4] < 1 ) // 1 input are allowed 
				{
			...
			{
				JOptionPane.showMessageDialog(null, "For this Exp your don't need this component !! :)");
/*				if ( comp_count[5] < 1 ) // Only 1 components are allowed 
				{
				...
				{
					JOptionPane.showMessageDialog(null, "You already have One component !! :)");
				}
*/		
			}
			...
			{
			//	JOptionPane.showMessageDialog(null, "For this Exp your don't need this component !! :)");
				if ( comp_count[8] < 1 ) // Only 1 components are allowed 
				{
			...
			else if (e.getSource() == img_button2[5] ) // Inverter 
			{
				if ( comp_count[11] < 5 ) // Only 5 components are allowed 
				{
				...
				{
					JOptionPane.showMessageDialog(null, "You already have Five of these component !! :)");
				}
			}
