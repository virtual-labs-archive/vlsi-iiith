...
public class exp5_mux extends JApplet 
{
	public  void init()
	{
	...
	public  class MyPanel extends JPanel  implements ActionListener , MouseMotionListener 
	{
		...
		int exp_type = 1 ; // 0 -> PASS TRANSISTOR/ MUX 
		...
		// MUX1 = 0-3 ; MUX2 = 4-7 ; OUTPUT = 8 ; INPUT = In1:9 , In2: 10 , clk:11 , Clk' :12   ; 
					...
					for ( int k = 0 ; k < 4 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] + 5; i ++ )
						{
							for ( int j = end_pointsY[k] - 4 ; j < end_pointsY[k] +5; j ++ )
							{
								...
								end_points_mat[i][j] = k + 4*(comp_no-1 ); // 0-3 for  MUX1 ;  4-7 for MUX2
							}
						}
					}
				}
				else if ( img == 4 || img == 10 ) //  INPUT , OUTPUT
				{
					...
					for ( int i = end_pointsX[0] - 4 ; i < end_pointsX[0] +5 ; i ++ )
					{
						for ( int j = end_pointsY[0] - 4 ; j < end_pointsY[0] +5; j ++ )
						{
							if ( img == 4 ) 				// INPUT
							{
								end_points_mat[i][j] = 8  + comp_no  ; // 9-12 ( In1 , In2 , Clk , Clk' )
			...
			*/	
			...
			public void make_end_points(int img )
			{
				...
				if ( img == 5 ) // MUX
				{
					...
					end_pointsX[2] = node_x + width / 2  ;
					end_pointsY[2] = node_y +  height ;

					end_pointsX[3] = node_x  ;
					end_pointsY[3] = node_y +  height / 2;
			...
			public void mouseClicked(MouseEvent me) 
			{
				...
				if ( img_button_pressed == -1 ) // for selecting anything on work panel
				{
					...
					else if ( work_mat[work_x][work_y]!= -1 ) // there is a comp on (work_x , work_y) here temp rep inder of comp_nodearray
					{
						...
						if (  temp1 == 1 || temp1 == 7 || temp1 == 8 || temp1 == 2 ||  temp1 == 9  ||( /*for Input  */temp1 == 4 && (comp_node[temp].comp_no == 1 || comp_node[temp].comp_no == 2 ||comp_node[temp].comp_no == 3 )))
						{
...
*/
				...
				else  // For adding comp 
				{
					...
					if ( img_button_pressed == 5) // MUX
					{
						comp_node[total_comp] = new node((work_x / grid_size)*grid_size , (work_y / grid_size )*grid_size , img_button_pressed , grid_size * 6 , 8 * grid_size , comp_count[5]+1);
					}
			...
			*/		//--------------------------------------------------------------------
			...
			//	draw_mux(g2d , 100 , 100 , grid_size , 0 ) ;
		...
		//For Images --------------------------------
				...
				for ( i = 0; i < total_comp ; i++ )
				{
					if ( comp_node[i].del != true )
					{
						...
						if ( comp_node[i].img_no == 5) // MUX 
						{
							draw_mux(g2d , comp_node[i].node_x  , comp_node[i].node_y , grid_size , comp_node[i].angle);
							g.setColor(Color.yellow);
							g.drawString(comp_str[5] + comp_node[i].comp_no, comp_node[i].node_x -10 , comp_node[i].node_y - 20 );
						}
...
//				g.drawRect( x , y , 2*width , 2*width);
			...
			void draw_input(Graphics2D g , int x , int y , int width , double angle , int comp_no)
			{
				...
				if ( comp_no == 1 )
				{
					g.drawString("IN 1" , x - 35 , y+ width - 2 );
				}
				else if ( comp_no == 2 )
				{
					g.drawString("IN 2", x -35 , y+ width );
				}
				else if ( comp_no == 3 )
				{
		...
		public class graph extends JPanel
		{
			...
			double[] V_in1 = new double[10000] ;
			double[] V_in2 = new double[10000] ;
			...
			public void make_graph()
			{
				...
				try{
					...
					while((line = bf.readLine()) != null){
						...
						line = bf.readLine();
						line = bf.readLine();
									V_in1[i] = Double.parseDouble(line);
						line = bf.readLine();
						line = bf.readLine();
									V_in2[i] = Double.parseDouble(line);
						line = bf.readLine();
						line = bf.readLine();
				...
				//	System.out.println("Hi I am in the gpaint func of the exp1_graph class :)");
					...
					{
					   //g2d.drawLine(40+2*i , 80-(int)Math.round(V_in[i]*60) , 40 + 2*(i+1) , 80-(int)Math.round(V_in[i+1]*60) );
						g2d.drawLine(40+2*i , 90-(int)Math.round(V_in1[i]*70) , 40 + 2*(i+1) , 90-(int)Math.round(V_in1[i+1]*70) );
					}
				//	g2d.drawLine(100, 40 , 150 , 40);
					g2d.drawString("In1 ",  4 , 50 );

					g2d.setColor(Color.blue);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
						//g2d.drawLine(40+2*i , 170-(int)Math.round(V_clk[i]*60) , 40 + 2*(i+1) , 170-(int)Math.round(V_clk[i+1]*60) );
					   g2d.drawLine(40+2*i , 200-(int)Math.round(V_in2[i]*70) , 40 + 2*(i+1) , 200-(int)Math.round(V_in2[i+1]*70) );
					}
				//	g2d.drawLine(100, 40 , 150 , 40);
					g2d.drawString("In2 ",  4 , 160 );

					g2d.setColor(Color.blue);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					//	g2d.drawLine(40+2*i , 260-(int)Math.round(V_clk1[i]*60) , 40 + 2*(i+1) , 260-(int)Math.round(V_clk1[i+1]*60) );
				...
				/*	g2d.setColor(Color.blue);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					   g2d.drawLine(40+2*i , 350-(int)Math.round(V_in[i]*60) , 40 + 2*(i+1) , 350-(int)Math.round(V_in[i+1]*60) );
					}
				//	g2d.drawLine(100, 40 , 150 , 40);
					g2d.drawString("In ",  4 , 320 );
				*/
					g2d.setColor(Color.red);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
...
//Setting Top Panel ========================================================================== 
			...
			JLabel heading = new JLabel (  "<html><FONT COLOR=WHITE SIZE=18 ><B>2-INPUT MULTIPLEXER</B></FONT></html>", JLabel.CENTER);
...
//			bottom.add(h );
		...
		public boolean circuit_check()
		{
			...
			int temp ;
			for (  i = 0 ; i < 13 ; i ++ ) // for making aal connected point for each end_point in its array ..
			{	
...
//========================================================================================================================================================
// ================================== exp5  ( Multiplexer )  circuit check ===========================================================
//========================================================================================================================================================
/*
...
*/			// Used Replaced array only for indexies ( 0-23 ) i.e NPOS and PMOS
			...
			{
				l = 0 ;
				while (  check_points_value[8][l] != -1 )  // OUT 
				{
					if ( check_points_value[8][l] != 8 && check_points_value[8][l] != 5 && check_points_value[8][l] !=  1 )
					{
				...
				l = 0 ;
				while (  check_points_value[9][l] != -1 )  // IN1
				{
				...
				l = 0 ;
				while (  check_points_value[10][l] != -1 )  // IN2
				{
					if ( check_points_value[10][l] != 10 && check_points_value[10][l] != 7 )
					{
				...
				l = 0 ;
				while (  check_points_value[11][l] != -1 )  // CLK
				{
					if ( check_points_value[11][l] != 11 &&  check_points_value[11][l] != 6 &&  check_points_value[11][l] != 0 )
					{
				...
				l = 0 ;
				while (  check_points_value[12][l] != -1 )  // CLK'
				{
					if ( check_points_value[12][l] != 12 &&  check_points_value[12][l] != 2 &&  check_points_value[12][l] != 4 )
					{
							System.out.println("clk'");			
						return false ;
					}
		...
		public void actionPerformed(ActionEvent e )
		{
			...
			if ( e.getSource() == exp_list )
			{
				...
				else 
				{
					...
					try
					{
						...
						URL url = new URL(base ,"exp5_pass.html");
			...
			else if(e.getSource() == simulate_button )
			{
				...
				if (circuit_check())
				{
					...
					String      encoded = "comp=" + URLEncoder.encode(MinVolt1+"_"+MaxVolt1+"_"+RiseTime1+"_"+FallTime1+"_"+StepSize1+"_"+TotalTime1+"_"+MinVolt2+"_"+MaxVolt2+"_"+RiseTime2+"_"+FallTime2+"_"+StepSize2+"_"+TotalTime2+"_"+MinVolt3+"_"+MaxVolt3+"_"+RiseTime3+"_"+FallTime3+"_"+StepSize3+"_"+TotalTime3); 
					URL CGIurl = null;
					try
					{
						if ( exp_type == 0 )
...
//				if ( simulate_flag == true )
		...
		//		}
			...
			{
				if ( comp_count[4] < 4 ) // 4 inputs are allowed 
				{
			...
			{
//				JOptionPane.showMessageDialog(null, "For this Exp your don't need this component !! :)");
				if ( comp_count[5] < 2 ) // Only 2 components are allowed 
				{
