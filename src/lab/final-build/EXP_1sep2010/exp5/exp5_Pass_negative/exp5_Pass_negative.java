...
public class exp5_Pass_negative extends JApplet 
{
	public  void init()
	{
	...
	public  class MyPanel extends JPanel  implements ActionListener , MouseMotionListener 
	{
		...
		int exp_type = 0 ; // 0 -> Pass Negative
		...
		// PMOS = 1-2 ; NMOS = 3-5 ; INPUT = In:6 , clk:7 , Clk' :8   ; OUTPUT = 9 ;
		//----------------------------------------------------------------------------------------------------------------------------------------
				...
				{	
					for ( int k = 0 ; k < 3 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] + 5; i ++ )
						{
							...
							{
								end_points_mat[i][j] = k ; // 0-2 for  PMOS 
							}
						}
					}
				}
				else if ( img == 7)// Nmos
				{
					for ( int k = 0 ; k < 3 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] +5; i ++ )
						{
							for ( int j = end_pointsY[k] - 4 ; j < end_pointsY[k] + 5; j ++ )
							{
								end_points_mat[i][j] = k +  3 ; // 3-5 for  NMOS 
							}
						}
					}
				}
				else if ( img == 2 || img == 9 || img == 4 || img == 10 ) // ground , VDD , INPUT , OUTPUT
				{
					...
					for ( int i = end_pointsX[0] - 4 ; i < end_pointsX[0] +5 ; i ++ )
					{
						for ( int j = end_pointsY[0] - 4 ; j < end_pointsY[0] +5; j ++ )
						{
							...
							if ( img == 4 ) 				// INPUT
							{
								end_points_mat[i][j] =  5 + comp_no  ; // 6-8 ( In , Clk , Clk' ) as (comp_no is 1-3)
...
//					wire[total_wire-1 ].update2((work_x/20)*20 , (work_y/20)*20);
			...
			public void mouseDragged(MouseEvent me)
			{
				...
				else 
				{
					...
					// for boundary check even if rotated (by adding heights Or width which ever is maxima )
					int t1 = comp_node[total_comp-1].height + grid_size;
					if ( comp_node[total_comp-1].width +grid_size > t1 )
					{
						t1 = comp_node[total_comp-1].width + grid_size;
					}
			...
			public void mouseClicked(MouseEvent me) 
			{
				...
				if ( img_button_pressed == -1 ) // for selecting anything on work panel
				{
					...
					else if ( work_mat[work_x][work_y]!= -1 ) // there is a comp on (work_x , work_y) here temp rep inder of comp_nodearray
					{
						...
						if (  temp1 == 1 || temp1 == 7 || temp1 == 8 || temp1 == 2 ||  temp1 == 9  ||( /*for Input  */temp1 == 4 && (comp_node[temp].comp_no == 1 || comp_node[temp].comp_no == 2 )))
						{
...
//				g.drawRect( x , y , 2*width , 2*width);
		...
		{
			String fileToRead = "outfile_n_pass";
			...
			double[] V_clk1 = new double[10000] ; // complement of clk
		...
		//public  graph()
			...
			public void make_graph()
			{
				...
				try
				{
					...
					System.out.println("Did't got the outfile to read :(");
				}
				String line;
				try{
					...
					while((line = bf.readLine()) != null){
						...
						line = bf.readLine();
						line = bf.readLine();
						V_clk1[i] = Double.parseDouble(line);
				...
				// Graph Plots =======================================================================
					...
					{
						g2d.drawLine(40+2*i , 150-(int)Math.round(V_in[i]*90) , 40 + 2*(i+1) , 150-(int)Math.round(V_in[i+1]*90) );
					}
				
					g2d.drawString(" In ",  4 , 100 );

					g2d.setColor(Color.blue);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					   g2d.drawLine(40+2*i , 300-(int)Math.round(V_clk[i]*90) , 40 + 2*(i+1) , 300-(int)Math.round(V_clk[i+1]*90) );
					}
				
					g2d.drawString("Clk ",  4 , 260 );


					g2d.setColor(Color.red);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					   g2d.drawLine(40+2*i , 440-(int)Math.round(V_out[i]*90) , 40 + 2*(i+1) , 440-(int)Math.round(V_out[i+1]*90) );
					}
				
					g2d.drawString("Out",  8 , 400 );
...
//Setting Top Panel ========================================================================== 
			...
			JLabel heading = new JLabel (  "<html><FONT COLOR=WHITE SIZE=18 ><B> NEGATIVE LEVEL PASS TRANSISTOR </B></FONT></html>", JLabel.CENTER);
			...
			String[] my_list = {"Negative Level Pass Transistor Circuit " };//, "2-Input Multiplexer Circuit"};
...
//			bottom.add(h );
		...
		{
			int check_points_value[][] = new int[10][10] ; // each index will store corresponding  component for points of (comp point no -> index )
...
//========================================================================================================================================================
// ================================== exp5 (Pass Transistor) / ( Multiplexer )  circuit check ===========================================================
//========================================================================================================================================================
/*
...
*/			// Used Replaced array only for indexies ( 0-23 ) i.e NPOS and PMOS
			...
			if ( exp_type == 0 ) // for Pass Transistor  
			{
				l = 0 ;
				while (  check_points_value[0][l] != -1 )  // OUT 
				{
					if ( check_points_value[0][l] != 0 && check_points_value[0][l] != 4 && check_points_value[0][l] !=  9 )
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[1][l] != -1 )  // IN
				{
					if ( check_points_value[1][l] != 1 &&  check_points_value[1][l] != 3 && check_points_value[1][l] != 6 )
					{
				...
				l = 0 ;
				while (  check_points_value[2][l] != -1 )  // CLK
				{
					if ( check_points_value[2][l] != 2 && check_points_value[2][l] != 7 )
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[5][l] != -1 )  // CLK'
				{
					if ( check_points_value[5][l] != 5 &&  check_points_value[5][l] != 8 )
					{
						return false ;
					}
					l++;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//------------------------------======Multiplexer ======( NOT applicable Here )---------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------
			else if ( exp_type == 1 ) // Multiplexer 
			{
				/*
				l = 0 ;
		...
		public void actionPerformed(ActionEvent e )
		{
			...
			if ( e.getSource() == exp_list )
			{
				...
				// Move to the Mux exp =========================================
					...
					try
					{
						...
						URL url = new URL(base ,"exp5_Pass_negative.html");
			...
			*/
				if ( Pmos1_l != null && Pmos1_w != null && Nmos1_l != null && Nmos1_w != null && circuit_check())
				{
					
					URL php_file =null ;
					URLConnection c =null;
					String      encoded = "comp=" + URLEncoder.encode( Pmos1_l+"_"+Pmos1_w+"_"+Nmos1_l+"_"+Nmos1_w+"_"+MinVolt1+"_"+MaxVolt1+"_"+RiseTime1+"_"+FallTime1+"_"+StepSize1+"_"+TotalTime1+"_"+MinVolt3+"_"+MaxVolt3+"_"+RiseTime3+"_"+FallTime3+"_"+StepSize3+"_"+TotalTime3); 
					URL CGIurl = null;
					try
					{
						if ( exp_type == 0 )
						{
							php_file = new URL(getDocumentBase(),"exp5_Pass_negative_simulate.php");
					...
					simulate_flag = true ;  //Used by the layout button 
				}
				else
				{
