		...
		public  class WorkPanel extends JPanel  implements  MouseMotionListener , MouseListener   
		{
			...
			public void mouseClicked(MouseEvent me) 
			{
				...
				if ( img_button_pressed == -1 ) // for selecting anything on work panel
				{
					...
					if (  temp != -1 )  // there is a comp on (work_x , work_y) here temp rep inder of comp_node array
					{ 
			...
			//	draw_ground(g2d , 400 , 20 , 20);
				...
				for ( i = 0 ; i < total_wire ; i++ )
				{
					...
					g2d.drawLine (wire[i].x1 , wire[i].y1 , wire[i].x1 , wire[i].y2 );
					g2d.drawLine (wire[i].x1 , wire[i].y2 , wire[i].x2 , wire[i].y2 );

					g2d.setColor(Color.red);
					g2d.fillRect (wire[i].x1 -2  , wire[i].y1 -3 , 6 ,6);
					g2d.fillRect (wire[i].x2 - 2 , wire[i].y2 -3 , 6 ,6);
