...
public class exp6_latch_negative extends JApplet 
{
	public  void init()
	{
...
//				g.drawRect( x , y , 2*width , 2*width);
		...
		{
			String fileToRead = "outfile_latch_n";
			...
			public void make_graph()
			{
				if ( exp_type == 0 )
				{
					fileToRead ="outfile_flipflop_n";
				}
				else if ( exp_type == 1 )
				{
						fileToRead ="outfile_latch_n";
				}
				URL url = null;
				try
				{
...
//Setting Top Panel ========================================================================== 
			...
			JLabel heading = new JLabel (  "<html><FONT COLOR=WHITE SIZE=18 ><B> D-LATCH : Negative Edge Trigger</B></FONT></html>", JLabel.CENTER);
			...
			String[] my_list = {"D Flip-Flop : Negative Edge " , "D-Latch : Negative Edge"};
			...
			if ( e.getSource() == exp_list )
			{
				...
				else 
				{
					...
					try
					{
						...
						URL url = new URL(base ,"exp6_flipflop_negative.html");
			...
			else if(e.getSource() == simulate_button )
			{
				...
				if (true )//circuit_check())
				{
					...
					try
					{
						if ( exp_type == 0 )
						{
							php_file = new URL(getDocumentBase(),"exp6_flipflop_negative_simulate.php");
							System.out.println( php_file.toString());
						}
						else if ( exp_type == 1 )
						{
							php_file = new URL(getDocumentBase(),"exp6_latch_negative_simulate.php");
							System.out.println( php_file.toString());
						}
					}
					catch(Exception mye )
			...
			else if(e.getSource() == graph_button )
			{
				...
				if ( simulate_flag == true )
				{
					try
					{
						...
						if ( exp_type == 0 )
						{
							url = new URL(base ,"exp6_flipflop_negative_fullgraph.php");
						}
						else if ( exp_type == 1 )
						{
							url = new URL(base ,"exp6_latch_negative_fullgraph.php");
						}
