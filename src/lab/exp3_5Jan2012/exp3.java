...
public class exp3 extends JApplet 
{
	...
	public  class MyPanel extends JPanel  implements ActionListener , MouseMotionListener 
	{
		...
		int grid_size = 16 ; // size of each box in grid 
		...
		// NMOS = 0-11 ; PMOS = 12-23 ; INPUT = a:24 , a':25 , b:26 ,b':27 ; OUTPUT = 28 ; GND = 29 ; VDD = 30 ; CAPACITOR = 31,32 ......
		//----------------------------------------------------------------------------------------------------------------------------------------
...
// For Input B		
		...
		public 	class node 
		{
			...
			{
			// PMOS = 12-23 ; NMOS = 0-11 ; INPUT = a:24 , a':25 , b:26 ,b':27 ; OUTPUT = 28 ; GND = 29 ; VDD = 30 ; CAPACITOR = 31,32 ......
				if ( img == 7) // nmos
				{	
					for ( int k = 0 ; k < 3 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] + 5; i ++ )
						{
							...
							{
								end_points_mat[i][j] = k + (3 * (comp_no -1) ); // 0-11 for all PMOS ( as comp_no is 1-4 )
							}
						}
					}
				}
				else if ( img == 1)// pmos
				{
					for ( int k = 0 ; k < 3 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] +5; i ++ )
						{
							for ( int j = end_pointsY[k] - 4 ; j < end_pointsY[k] + 5; j ++ )
							{
								end_points_mat[i][j] = k + ( 3 * (comp_no + 4 - 1) ); // 12-23 for all NMOS (as comp_no is 1-4 
							}
						}
					}
				}
				else if ( img == 8 ) // Capacitor 
				{
					for ( int k = 0 ; k < 2 ; k ++ )
					{
						for ( int i = end_pointsX[k] - 4 ; i < end_pointsX[k] +5 ; i ++ )
						{
							for ( int j = end_pointsY[k] - 4 ; j < end_pointsY[k] +5; j ++ )
							{
								end_points_mat[i][j] = k + 31; // 31 and 32 
							}
						}
					}
				}
				else if ( img == 2 || img == 9 || img == 4 || img == 10 ) // ground , VDD , INPUT , OUTPUT
				{
					...
					for ( int i = end_pointsX[0] - 4 ; i < end_pointsX[0] +5 ; i ++ )
					{
						for ( int j = end_pointsY[0] - 4 ; j < end_pointsY[0] +5; j ++ )
						{
							if ( img == 2 ) 				// ground
							{
								end_points_mat[i][j] =  29;
							}
							else if ( img == 9 ) 				//vdd
							{
								end_points_mat[i][j] =  30;
							}
							else if ( img == 4 ) 				// INPUT
							{
								end_points_mat[i][j] =  24 + comp_no - 1 ; // 24-27 ( A , A' , B , B') as (comp_no is 1-4)
								
							}
							else 						// OUTPUT 
							{
								end_points_mat[i][j] =  28;
							}
						}
					}
				}
			...
			public void actionPerformed(ActionEvent e )
			{
				if(e.getSource() == ok )
				{
					...
					if ( comp_node[node_index].img_no  == 1 ) //PMOS
					{
						...
						else if ( comp_node[node_index].comp_no == 3 ) 
						{
							Pmos3_l = get_length();
							Pmos3_w = get_width();
						}
						else if ( comp_node[node_index].comp_no == 4 ) 
						{
							Pmos4_l = get_length();
							Pmos4_w = get_width();
						}
			...
			public void mouseClicked(MouseEvent me) 
			{
				...
				if ( img_button_pressed == -1 ) // for selecting anything on work panel
				{
					...
					else if ( work_mat[work_x][work_y]!= -1 ) // there is a comp on (work_x , work_y) here temp rep inder of comp_nodearray
					{
						...
						if (  temp1 == 1 || temp1 == 7 || temp1 == 8 || temp1 == 2 ||  temp1 == 9  ||( temp1 == 4 && (comp_node[temp].comp_no == 1 || comp_node[temp].comp_no == 3 )))
						{
...
//				g.drawRect( x , y , 2*width , 2*width);
			...
			void draw_output(Graphics2D g , int x , int y , int width , double angle)
			{
				...
				g.rotate(angle , x , y);
				g.setColor(Color.yellow);
				g.drawString("Out", x  + width /2 , y+ width - 3);
		...
		public class graph extends JPanel
		{
			...
			double[] Va_in = new double[10000] ;
			double[] Vb_in = new double[10000] ;
			double[] Va1_in = new double[10000] ; // complement of a
			double[] Vb1_in = new double[10000] ; // Complement of b
			...
			public void make_graph()
			{
				if ( exp_type == 0 )
				{
					fileToRead ="outfile_xor";
				}
				else if ( exp_type == 1 )
				{
						fileToRead ="outfile_xnor";
				}
				URL url = null;
				try
				{
				...
				try{
					...
					while((line = bf.readLine()) != null){
						...
						line = bf.readLine();
						line = bf.readLine();
						Va1_in[i] = Double.parseDouble(line);
						line = bf.readLine();
						line = bf.readLine();
						Vb1_in[i] = Double.parseDouble(line);
						i++;
					}
					no_values = i ;
				...
				//	System.out.println("Hi I am in the gpaint func of the exp1_graph class :)");
					...
					{
					   g2d.drawLine(40+2*i , 80-(int)Math.round(Va_in[i]*60) , 40 + 2*(i+1) , 80-(int)Math.round(Va_in[i+1]*60) );
					}
				//	g2d.drawLine(100, 40 , 150 , 40);
					g2d.drawString("In-a ",  4 , 50 );

					g2d.setColor(Color.blue);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					   g2d.drawLine(40+2*i , 170-(int)Math.round(Va1_in[i]*60) , 40 + 2*(i+1) , 170-(int)Math.round(Va1_in[i+1]*60) );
					}
				//	g2d.drawLine(100, 40 , 150 , 40);
					g2d.drawString("In-a' ",  4 , 140 );

					g2d.setColor(Color.blue);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					   g2d.drawLine(40+2*i , 260-(int)Math.round(Vb_in[i]*60) , 40 + 2*(i+1) , 260-(int)Math.round(Vb_in[i+1]*60) );
					}
				//	g2d.drawLine(100, 40 , 150 , 40);
					g2d.drawString("In-b ",  4 , 230 );

					g2d.setColor(Color.blue);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					   g2d.drawLine(40+2*i , 350-(int)Math.round(Vb1_in[i]*60) , 40 + 2*(i+1) , 350-(int)Math.round(Vb1_in[i+1]*60) );
					}
				//	g2d.drawLine(100, 40 , 150 , 40);
					g2d.drawString("In-b' ",  4 , 330 );

					g2d.setColor(Color.red);
					for( i = 0 ; i < no_values - 1 ; i++ )
					{
					   g2d.drawLine(40+2*i , 440-(int)Math.round(V_out[i]*60) , 40 + 2*(i+1) , 440-(int)Math.round(V_out[i+1]*60) );
					}
				//	g2d.drawLine( 100 , 260 , 150 , 260);
					g2d.drawString("Out",  8 , 420 );
...
//Setting Top Panel ========================================================================== 
			...
			JLabel heading = new JLabel (  "<html><FONT COLOR=WHITE SIZE=18 ><B>EXPERIMENT ON XOR/XNOR GATES </B></FONT></html>", JLabel.CENTER);
			...
			String[] my_list = {"XOR Gate Circuit " , "XNOR Gate Circuit"};
...
//			bottom.add(h );
		...
		{
			int check_points_value[][] = new int[33][33] ; // each index will store corresponding  component for points of (comp point no -> index )
			...
			int temp ;
			for (  i = 0 ; i < 33 ; i ++ ) // for making aal connected point for each end_point in its array ..
			{	
...
//                         -------------------------			need to wrtie this part of code
	...
	//--------------------------------------------------------------------------------
			...
			if ( exp_type == 0 ) // for XOR 
			{
				l = 0 ;
				while (  check_points_value[29][l] != -1 )
				{
					if ( check_points_value[29][l] != 29 && check_points_value[29][l] != replace[7] && check_points_value[29][l] != 10&& check_points_value[29][l] != 32 && check_points_value[29][l] != 31)
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[replace[2]][l] != -1 )
				{
					if ( check_points_value[replace[2]][l] != replace[2] &&  check_points_value[replace[2]][l] != 24 && check_points_value[replace[2]][l] != replace[14] )
					{
				...
				l = 0 ;
				while (  check_points_value[replace[5]][l] != -1 )
				{
					if ( check_points_value[replace[5]][l] != replace[5] && check_points_value[replace[5]][l] != 25 && check_points_value[replace[5]][l] != replace[17])
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[27][l] != -1 )
				{
					if ( check_points_value[27][l] != 27 &&  check_points_value[27][l] != replace[11]  &&  check_points_value[27][l] != 23  )
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[26][l] != -1 )
				{
					if ( check_points_value[26][l] != replace[8] && check_points_value[26][l] != 26 && check_points_value[26][l] != replace[20])
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[30][l] != -1 )
				{
					if ( check_points_value[30][l] != 30 && check_points_value[30][l] != replace[12] && check_points_value[30][l] != replace[18] )
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[28][l] != -1 )
				{
					if ( check_points_value[28][l] != 28 && check_points_value[28][l] != 31 && check_points_value[28][l] != 32 && check_points_value[28][l] != replace[3] && check_points_value[28][l] != replace[22] && check_points_value[28][l] != replace[16] && check_points_value[28][l] != 0 )
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[replace[13]][l] != -1 )
				{
					if ( check_points_value[replace[13]][l] != replace[13] && check_points_value[replace[13]][l] != replace[15] && check_points_value[replace[13]][l] != replace[21] && check_points_value[replace[13]][l] != replace[19] )
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[replace[1]][l] != -1 )
				{
					if ( check_points_value[replace[1]][l] != replace[1] && check_points_value[replace[1]][l] != replace[6])
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[replace[4]][l] != -1 )
				{
					if ( check_points_value[replace[4]][l] != replace[4] && check_points_value[replace[4]][l] != replace[9])
					{
						return false ;
					}
					l++;
...
//------------------------------======XNOR====== ---------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------
			else if ( exp_type == 1 ) // XNOR 
			{
				
				l = 0 ;
				while (  check_points_value[29][l] != -1 )
				{
					if ( check_points_value[29][l] != 29 && check_points_value[29][l] != 7 && check_points_value[29][l] != 10&& check_points_value[29][l] != 32 && check_points_value[29][l] != 31)
					{
						return false ;
					}
					l++;
				...
				{
					if ( check_points_value[2][l] != 2 &&  check_points_value[2][l] != 24 && check_points_value[2][l] != 14 )
					{
				...
				{
					if ( check_points_value[5][l] != 5 && check_points_value[5][l] != 25 && check_points_value[5][l] != 17)
					{
						return false ;
					}
					l++;
				...
				while (  check_points_value[26][l] != -1 )
				{
					if ( check_points_value[26][l] != 26 &&  check_points_value[26][l] != 11  &&  check_points_value[26][l] != 23  )
					{
						return false ;
					}
					l++;
				...
				{
					if ( check_points_value[8][l] != 8 && check_points_value[8][l] != 26 && check_points_value[8][l] != 20)
					{
						return false ;
					}
					l++;
				...
				while (  check_points_value[30][l] != -1 )
				{
					if ( check_points_value[30][l] != 30 && check_points_value[30][l] != 12 && check_points_value[30][l] != 18 )
					{
						return false ;
					}
					l++;
				...
				while (  check_points_value[28][l] != -1 )
				{
					if ( check_points_value[28][l] != 28 && check_points_value[28][l] != 31 && check_points_value[28][l] != 32 && check_points_value[28][l] != 3 && check_points_value[28][l] != 22 && check_points_value[28][l] != 16 && check_points_value[28][l] != 0 )
					{
						return false ;
					}
					l++;
				...
				l = 0 ;
				while (  check_points_value[13][l] != -1 )
				{
					if ( check_points_value[13][l] != 13 && check_points_value[13][l] != 15 && check_points_value[13][l] != 21 && check_points_value[13][l] != 19 )
					{
						return false ;
					}
					l++;
				...
				{
					if ( check_points_value[1][l] != 1 && check_points_value[1][l] != 6)
					{
						return false ;
					}
					l++;
				...
				{
					if ( check_points_value[4][l] != 4 && check_points_value[4][l] != 9)
					{
						return false ;
					}
					l++;
		...
		public void actionPerformed(ActionEvent e )
		{
			...
			else if(e.getSource() == simulate_button )
			{
				...
				if ( Pmos1_l != null && Pmos1_w != null && Nmos1_l != null && Nmos1_w != null && Pmos2_l != null && Pmos2_w != null && Nmos2_l != null && Nmos2_w != null &&  Capacitance != null  && circuit_check())
				{
					...
					String      encoded = "comp=" + URLEncoder.encode( Pmos1_l+"_"+Pmos1_w+"_"+Nmos1_l+"_"+Nmos1_w+"_"+Pmos2_l+"_"+Pmos2_w+"_"+Nmos2_l+"_"+Nmos2_w+"_"+Capacitance+"_"+MinVolt1+"_"+MaxVolt1+"_"+RiseTime1+"_"+FallTime1+"_"+StepSize1+"_"+TotalTime1+"_"+MinVolt3+"_"+MaxVolt3+"_"+RiseTime3+"_"+FallTime3+"_"+StepSize3+"_"+TotalTime3); 
					URL CGIurl = null;
					try
					{
						if ( exp_type == 0 )
						{
							php_file = new URL(getDocumentBase(),"exp3_xor_simulate.php");
							System.out.println( php_file.toString());
						}
						else if ( exp_type == 1 )
						{
							php_file = new URL(getDocumentBase(),"exp3_xnor_simulate.php");
							System.out.println( php_file.toString());
						}
					}
					catch(Exception mye )
...
//				if ( simulate_flag == true )
		...
		//		}
			...
			else if(e.getSource() == graph_button )
			{
				...
				if ( simulate_flag == true )
				{
					try
					{
						...
						if ( exp_type == 1 )
						{
							url = new URL(base ,"exp3_xor_fullgraph.php");
						}
						else if ( exp_type == 2 )
						{
							url = new URL(base ,"exp3_xnor_fullgraph.php");
						}
						getAppletContext().showDocument(url , "Output Exp 3 ");
					}
					catch(Exception ee )
					{}
				}
			...
			else if (e.getSource() == img_button1[1] )
			{
				...
				{
					JOptionPane.showMessageDialog(null, "You already have four of these component !! :)");
				}
