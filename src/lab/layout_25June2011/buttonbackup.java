...
/*				g.setStroke(new BasicStroke(2));
			...
			void stretch(Graphics2D g,int x , int y ,int width, double angle)
			{
				int mouse_x;
				int mouse_y;
				mouse_x = MouseInfo.getPointerInfo().getLocation().x;
				mouse_y = MouseInfo.getPointerInfo().getLocation().y;
				draw_active(g , x , y , width + (x - mouse_x),angle);
...
//				System.out.println("Mouse Cord"+work_x+work_y);
/*				g.setStroke(new BasicStroke(2));
				g.drawLine(x  , y + 2*width , x + width , y + 2*width);
 ...
 */			
			...
			java.net.URL imgURL3 = getClass().getResource("images/simulate.png");
			JPanel rightButton = new JPanel (new FlowLayout(FlowLayout.CENTER , 10 , 1 )) ;
//			JLabel wave_head = new JLabel("This is the wave simulation output");
			JLabel wave_head = new JLabel ( "<html><FONT COLOR=white SIZE=6 ><B>NEW OPTIONS TO TEST</B></FONT><br><br></html>", JLabel.CENTER);
			wave_head.setBorder(BorderFactory.createRaisedBevelBorder( ));

			stretch_button = new JButton(" STRETCH " , icon_stretch );
			icon_stretch.setImageObserver(stretch_button);
			
			rightPanel.setBackground(Color.gray);

			//waveRightPanel = new graph() ;// = new exp1_graph();
			rightButton.add(stretch_button );
			//rightPanel.add(waveRightPanel, BorderLayout.CENTER);
			//rightPanel.add(wave_head, BorderLayout.NORTH);

			if (imgURL3 != null) 
			{
				icon_stretch =  new ImageIcon(imgURL3);
			}
			else
			{
				icon_stretch = null ;	
