...
public class exp1 extends JApplet 
{
	...
	public  class MyPanel extends JPanel  implements ActionListener , MouseMotionListener 
	{
		...
		String[] comp_str = {		// This will be shown on tool tip
			...
			"Active ", "Metal " ," Poly Contact ",  // 1, 2 , 3
			"N Well " ,"N Select ", " P Select " , // 4 , 5 ,6

			"Metal contact ", "Metal 1" ,"Metal 3 ",  // 7 , 8 , 9 
			"Via"};
		public 	class node 
		{
			int node_x ;
		...
		public  class WorkPanel extends JPanel  implements  MouseMotionListener , MouseListener   
		{
			...
			public void mouseDragged(MouseEvent me)
			{
				...
				else if (  comp_pick_mode == 0 && img_button_pressed == -1 && selected_mat != 0 ) // Move  is allowed 
				{
				...
				for ( i = 1; i <= total_comp_type  ; i++ )
				{
					for ( j = 0 ; j < comp_count[i] ; j ++ )
					{
						if ( each_comp_node_array[i][j].del == false )
						{
							...
							{
								draw_contact(g2d , each_comp_node_array[i][j].node_x, each_comp_node_array[i][j].node_y  , each_comp_node_array[i][j].width , each_comp_node_array[i][j].height);
							}
				
							else if ( each_comp_node_array[i][j].img_no == 8)
							{
								draw_metal2(g2d , each_comp_node_array[i][j].node_x, each_comp_node_array[i][j].node_y  , each_comp_node_array[i][j].width , each_comp_node_array[i][j].height);
...
//=============================================================================================
		...
		public void actionPerformed(ActionEvent e )
		{
			...
			if(e.getSource() == fix_size_button )
			{
				...
				if ( true )//comp property is satisfied 
				{
					change_selected(0);
					comp_pick_mode = 0 ;
